...
def enviar_bolos():
    ...
    agd = agenda_151(baixar_relatorio(274))
    ...
    for loja, emails in ccp_sem_digito.items():
        ...
        if dia_da_semana == 'Segunda':
            ...
            titulo = f"BAIXAS DE BOLOS - LOJA {loja} - {sexta.strftime('%d')}, {anteontem.strftime('%d')} E {ontem.strftime('%d/%m')}"
            tal_dia = f'{sexta.strftime('%d')}, {anteontem.strftime('%d')} e {ontem.strftime('%d/%m/%Y')}'
        
        else:

            titulo = f"BAIXAS DE BOLOS - LOJA {loja} - {ontem.strftime('%d/%m')}"
